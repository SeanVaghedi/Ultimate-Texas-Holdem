"""Q-learning training driver for tabular UTH RL.

Run from the repo root:

    python -m src.training.train_q [--episodes N] [--output-suffix S]

Or directly:

    python src/training/train_q.py [--episodes N] [--output-suffix S]
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Q-learning UTH trainer.")
    parser.add_argument(
        "--episodes",
        type=int,
...
def main() -> None:
    ...
    alpha = args.alpha
    output_qtable = f"results/q_qtable{suffix}.npz"
    output_log = f"results/q_training_log{suffix}.csv"
    ...
    q_table = QTable()
    agent = QLearningAgent(q_table, rng, alpha=alpha, gamma=GAMMA)
