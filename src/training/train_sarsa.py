"""SARSA training driver for tabular UTH RL.

Run from the repo root:

    python -m src.training.train_sarsa [--episodes N] [--output-suffix S]

Or directly:

    python src/training/train_sarsa.py [--episodes N] [--output-suffix S]
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SARSA UTH trainer.")
    parser.add_argument(
        "--episodes",
        type=int,
...
def main() -> None:
    ...
    alpha = args.alpha
    output_qtable = f"results/sarsa_qtable{suffix}.npz"
    output_log = f"results/sarsa_training_log{suffix}.csv"

    print("=== SARSA Training Run ===")
    ...
    q_table = QTable()
    agent = SarsaAgent(q_table, rng, alpha=alpha, gamma=GAMMA)
