"""SARSA(lambda) training driver for tabular UTH RL.

Run from the repo root:

    python -m src.training.train_sarsa_lambda [--episodes N] [--output-suffix S]
        [--alpha A] [--lam L]

Or directly:

    python src/training/train_sarsa_lambda.py [--episodes N] [--output-suffix S]
        [--alpha A] [--lam L]
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SARSA(lambda) UTH trainer."
    )
    parser.add_argument(
        "--episodes",
        type=int,
    ...
    parser.add_argument(
        ...
        help="TD learning rate alpha (default: %(default)s).",
    )
    parser.add_argument(
        "--lam",
        type=float,
        default=DEFAULT_LAMBDA,
        help="Eligibility-trace decay lambda (default: %(default)s).",
    )
...
def main() -> None:
    ...
    alpha = args.alpha
    lam = args.lam
    output_qtable = f"results/sarsa_lambda_qtable{suffix}.npz"
    output_log = f"results/sarsa_lambda_log{suffix}.csv"

    print("=== SARSA(lambda) Training Run ===")
    ...
    q_table = QTable()
    agent = SarsaLambdaAgent(q_table, rng, alpha=alpha, gamma=GAMMA, lam=lam)
